var Vineyard = (function () {
  ...
  var Vine = Bloom.Flower.subclass('Vine', {
    initialize: function() {
      ...
      if (this.element) {
        ...
        label.text(Vine.pretty_name(this.name));
      }
    }
  });
  ...
  var List_Vine = Vine.subclass('List_Vine', {
    ...
    initialize: function() {
      ...
      var list = this.list_type.create(this.seed, this.element.find('ul'));
    ...
    create_new_entry: function(main_list) {
      ...
      var arbor = arbor_type.create({
        seed: Seed.create({}, this.trellis),
        trellis: this.property.reference_trellis
      });
