var Vineyard = (function () {
    ...
    initialize_properties: function() {            
      for (var p in this.properties) {
        ...
        if (property.type == 'list' || property.type == 'reference') {
          // Convert string to object reference.
          property.target_trellis = this.vineyard.trellises[property.reference_class]
        }
  ...
  var Seed = Meta_Object.subclass('Seed', {
    ...
    plant: function() {
      ...
      Bloom.post(this.trellis.vineyard.update_url + '?XDEBUG_SESSION_START=netbeans-xdebug', data, function() {
  ...
  var List_Vine = Vine.sub_class('List_Vine', {
    ...
    initialize: function() {
      ...
      this.element.find('.add').click(function(e) {
        e.preventDefault();
        Bloom.get(self.property.trellis.vineyard.get_url + '?trellis=' + self.property.reference_class, function(response) {          
