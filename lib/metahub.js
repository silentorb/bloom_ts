var MetaHub = (function () {
  ...
  var Meta_Object = MetaHub.Meta_Object = {
    ...
    create: function() {
      ...
      result.toString = function() { return this.type + ":" + this.guid; };
  ...
  if (SUPPORT_BAD_IE) {
    Meta_Object.properties.parent = function() {
      return this.get_connections('parent')[0];
    };
  }
  else {
    // I don't think there's any easy way to directly add getter/setters, 
    // so create a temporary object
    MetaHub.extend(Meta_Object, {
      get parent() {
        return this.get_connections('parent')[0];
      },
      set parent(new_parent) {
        this.connect(new_parent, 'parent', 'child');
      }
    });
  }
