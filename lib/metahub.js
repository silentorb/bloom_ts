var MetaHub = (function () {
    ...
    var Meta_Object = MetaHub.Meta_Object = {
        ...
        methods: {
            ...
            listen: function (other, name, method, options) {
            ...
            invoke_async: function (name) {
                ...
                var loop = function (x) {
                    if (x < info.length) {
                        ...
                        args[args.length - 1] = function() { loop(x + 1); }
                        info[x].method.apply(info[x].listener, args);
                    }
                    else {
                        if (typeof finish == 'function')
            ...
            parent_method: function (method_name) {
                var args = Array.prototype.slice.call(arguments, 1);
                return this.meta_source.parent.methods[method_name].apply(this, args);
            }
        }
    };
