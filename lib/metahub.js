var MetaHub = (function () {
  ...
  var Meta_Object = MetaHub.Meta_Object = {
    ...
    connect_objects: function(first, other, type) {
      if (first.connections[other]) {
        if (first.connections[other].type != type && type) { 
          first.connections[other].type = type;
          return true;
        }
      ...
      var connection = Meta_Connection.create(first, other, type);
      first.connections[other] = connection;              
      return true;
    },
    ...
    properties: {
      ...
      disconnect: function(other){
        if (this.connections[other]) {
          ...
          this.invoke('disconnect.' + type, other, this);

          if (connection.type == 'parent' && this.get_connections('parent').length == 0) {
            this.disconnect_all();
          }
        }      
      },      
      ...
      get_connections: function() {
        ...
        if (typeof first_filter == 'string') {
          for (var x in this.connections) {
            if (this.connections[x].type == first_filter) {
          ...
          for (var x in this.connections) {
            if (first_filter(this.connections[x].other)) {
              result.push(this.connections[x].other);
            }
          }
        }
        ...
        for (var f = 0; f < filters.length; f++) {
          ...
          if (typeof filter == 'string') {
            for (var x = result.length - 1; x >= 0; x--) {
              if (this.connections[result[x]].type == filter) {
                result.splice(x, 1);
              }
            }           
          }
